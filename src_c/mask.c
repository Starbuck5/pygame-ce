...
set_from_threshold(SDL_Surface *surf,
                   const SDL_PixelFormatDetails *surf_format,
                   SDL_Palette *surf_palette, bitmask_t *bitmask,
                   int threshold)
#else
set_from_threshold(SDL_Surface *surf, SDL_PixelFormat *surf_format,
                   SDL_Palette *surf_palette, bitmask_t *bitmask, int threshold)
#endif
{
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
                SDL_GetRGBA(bpp == 1 ? *srcp : *((Uint16 *)srcp), surf_format,
...
mask_from_surface(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    SDL_Palette *surf_palette = surf_format->palette;
#endif
                  ...
                  Uint32 color, Uint32 threshold, int palette_colors)
{
    ...
    // Declare for later
    const SDL_PixelFormatDetails *format2;
    ...
    if (surf2) {
#if SDL_VERSION_ATLEAST(3, 0, 0)
        format2 = SDL_GetPixelFormatDetails(surf2->format);
        palette2 = SDL_GetSurfacePalette(surf2);

        if (format2 == NULL) {
            PyErr_SetString(pgExc_SDLError, SDL_GetError());
            return -1;
        }
        rloss2 = format2->Rbits;
        gloss2 = format2->Gbits;
        bloss2 = format2->Bbits;
#else
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
    SDL_GetRGBA(color, format, palette, &r, &g, &b, &a);
    SDL_GetRGBA(threshold, format, palette, &tr, &tg, &tb, &ta);
#else
