...
image_frombuffer(PyObject *self, PyObject *arg, PyObject *kwds)
{
    ...
    if (!strcmp(format, "P")) {
        ...
        surf = PG_CreateSurfaceFrom(
            data, w, h, pitch,
            PG_GetPixelFormatEnumForMasks(32,
#if SDL_BYTEORDER == SDL_LIL_ENDIAN
                                          0xFF, 0xFF << 8, 0xFF << 16,
                                          (alphamult ? 0xFF << 24 : 0)));
#else
...
SaveTGA_RW(SDL_Surface *surface, SDL_RWops *out, int rle)
{
    ...
    if (!PG_RWWRITE(out, &h, sizeof(h), 1))
        return -1;
    ...
    if (h.has_cmap) {
        ...
        for (i = 0; i < pal->ncolors; i++) {
            ...
            if (!PG_RWWRITE(out, entry, h.cmap_bits >> 3, 1))
                return -1;
        }
    }

    linebuf = PG_CreateSurface(surface->w, 1,
                               PG_GetPixelFormatEnumForMasks(
                                   h.pixel_bits, rmask, gmask, bmask, amask));
    if (!linebuf)
        return -1;
