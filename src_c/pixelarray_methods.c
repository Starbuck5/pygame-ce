...
#define PXM_GET_PIXELVALS(_sR, _sG, _sB, _sA, px, fmt, palette, ppa) \
    SDL_GetRGBA(px, fmt, palette, &(_sR), &(_sG), &(_sB), &(_sA));   \
    if (!ppa) {                                                      \
        _sA = 255;                                                   \
    }
#else
...
_replace_color(pgPixelArrayObject *array, PyObject *args, PyObject *kwds)
{
    ...
    if (distance != 0.0) {
#if SDL_VERSION_ATLEAST(3, 0, 0)
        SDL_GetRGB(dcolor, format, palette, &r1, &g1, &b1);
#else
...
_extract_color(pgPixelArrayObject *array, PyObject *args, PyObject *kwds)
{
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
    black = SDL_MapRGBA(format, palette, 0, 0, 0, 255);
    white = SDL_MapRGBA(format, palette, 255, 255, 255, 255);
#else
    ...
    if (distance != 0.0) {
#if SDL_VERSION_ATLEAST(3, 0, 0)
        SDL_GetRGB(color, format, palette, &r1, &g1, &b1);
#else
...
_compare(pgPixelArrayObject *array, PyObject *args, PyObject *kwds)
{
    ...
    const SDL_PixelFormatDetails *format =
        SDL_GetPixelFormatDetails(surf->format);
    SDL_Palette *palette = SDL_GetSurfacePalette(surf);
    const SDL_PixelFormatDetails *other_format =
        SDL_GetPixelFormatDetails(other_surf->format);
    SDL_Palette *other_palette = SDL_GetSurfacePalette(other_surf);

    if (format == NULL || other_format == NULL) {
        return RAISE(pgExc_SDLError, SDL_GetError());
    }
#else
    SDL_PixelFormat *format = surf->format;
    SDL_Palette *palette = format->palette;
    SDL_PixelFormat *other_format = other_surf->format;
    SDL_Palette *other_palette = other_format->palette;
#endif
        ...
        default: /* case 4: */
        {
            ...
            for (y = 0; y < dim1; ++y) {
                ...
                for (x = 0; x < dim0; ++x) {
                    ...
                    if (distance != 0.0) {
                        PXM_GET_PIXELVALS(r1, g1, b1, a1, *pixel_p, format, palette, ppa);
