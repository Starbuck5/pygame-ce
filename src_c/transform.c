...
newsurf_fromsurf(SDL_Surface *surf, int width, int height)
{
    ...
    if (SDL_ISPIXELFORMAT_INDEXED(PG_SURF_FORMATENUM(surf))) {
        ...
        SDL_Palette *newsurf_palette = SDL_GetSurfacePalette(newsurf);
        SDL_Palette *surf_palette = SDL_GetSurfacePalette(newsurf);
...
#else
        SDL_Palette *newsurf_palette = newsurf->format->palette;
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
        const SDL_PixelFormatDetails *ret_format =
            SDL_GetPixelFormatDetails(retsurf->format);
...
#else
        SDL_PixelFormat *ret_format = retsurf->format;
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
_set_at_pixels(int x, int y, Uint8 *pixels,
               const SDL_PixelFormatDetails *format, int surf_pitch,
               Uint32 the_color)
#else
                ...
                Uint32 *color)
{
    if (color_obj) {
        ...
        SDL_PixelFormat *surf_format = surf->format;
#endif
        *color = SDL_MapRGBA(surf_format, rgba_default[0], rgba_default[1],
...
void
#if SDL_VERSION_ATLEAST(3, 0, 0)
grayscale_non_simd(SDL_Surface *src, const SDL_PixelFormatDetails *src_format,
                   SDL_Surface *newsurf,
                   const SDL_PixelFormatDetails *newsurf_format)
#else
grayscale_non_simd(SDL_Surface *src, SDL_PixelFormat *src_format,
                   SDL_Surface *newsurf, SDL_PixelFormat *newsurf_format)
#endif
{
    int x, y;
    for (y = 0; y < src->h; y++) {
        for (x = 0; x < src->w; x++) {
            ...
            Uint8 r, g, b, a;
#if SDL_VERSION_ATLEAST(3, 0, 0)
            SDL_GetRGBA(pixel, src_format, SDL_GetSurfacePalette(src), &r, &g,
                        &b, &a);
#else
            SDL_GetRGBA(pixel, src_format, &r, &g, &b, &a);
#endif
            ...
            Uint8 grayscale_pixel =
                (Uint8)((((76 * r) + 255) >> 8) + (((150 * g) + 255) >> 8) +
                        (((29 * b) + 255) >> 8));
#if SDL_VERSION_ATLEAST(3, 0, 0)
            Uint32 new_pixel = SDL_MapRGBA(
                newsurf_format, SDL_GetSurfacePalette(newsurf),
                grayscale_pixel, grayscale_pixel, grayscale_pixel, a);
#else
            Uint32 new_pixel =
                SDL_MapRGBA(newsurf_format, grayscale_pixel, grayscale_pixel,
                            grayscale_pixel, a);
#endif
...
grayscale(pgSurfaceObject *srcobj, pgSurfaceObject *dstobj)
{
    ...
    const SDL_PixelFormatDetails *src_format =
        SDL_GetPixelFormatDetails(src->format);
    const SDL_PixelFormatDetails *newsurf_format =
        SDL_GetPixelFormatDetails(newsurf->format);
...
#else
    SDL_PixelFormat *src_format = src->format;
    SDL_PixelFormat *newsurf_format = newsurf->format;
#endif
              ...
              const int keep_alpha)
{
    ...
    if (fmt == NULL || newsurf_format == NULL) {
        return (SDL_Surface *)(RAISE(pgExc_SDLError, SDL_GetError()));
    }
#else
    ...
    else /* path for 16 bit surfaces */
    {
        ...
        SDL_Palette *newsurf_palette = SDL_GetSurfacePalette(newsurf);
            ...
            SDL_GetRGBA(color, fmt, src_palette, &Cr, &Cg, &Cb, &Ca);
        const Uint16 color16 =
            (Uint16)SDL_MapRGBA(fmt, src_palette, Cr, Cg, Cb, Ca);
#else
        ...
        for (y = 0; y < src->h; y++) {
            for (x = 0; x < src->w; x++) {
#if SDL_VERSION_ATLEAST(3, 0, 0)
                SDL_GetRGBA((Uint32)*src_row, fmt, src_palette, &r, &g, &b,
                            &a);
#else
                ...
                if (a) {
                    if (keep_alpha)
                    // CHANGING here to use newsurf's format instead of srcs,
                    // for palette info potentially.
#if SDL_VERSION_ATLEAST(3, 0, 0)
                        *dst_row = (Uint16)SDL_MapRGBA(
                            newsurf_format, newsurf_palette, Cr, Cg, Cb, a);
#else
                        *dst_row =
                            (Uint16)SDL_MapRGBA(newsurf_format, Cr, Cg, Cb, a);
#endif
                    else
                        *dst_row = color16;
                }
...
static void
#if SDL_VERSION_ATLEAST(3, 0, 0)
modify_hsl(SDL_Surface *surf, const SDL_PixelFormatDetails *fmt,
           SDL_Surface *dst, const SDL_PixelFormatDetails *dst_format, float h,
           float s, float l)
#else
modify_hsl(SDL_Surface *surf, SDL_PixelFormat *fmt, SDL_Surface *dst,
           SDL_PixelFormat *dst_format, float h, float s, float l)
#endif
{
    int surf_locked = 0;
    ...
    SDL_Palette *dst_palette = SDL_GetSurfacePalette(surf);
#endif
    ...
    if (PG_FORMAT_BytesPerPixel(fmt) == 4 ||
        ...
        for (y = 0; y < surf->h; y++) {
            for (x = 0; x < surf->w; x++) {
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
                SDL_GetRGBA(pixel, fmt, surf_palette, &r, &g, &b, &a);
#else
                ...
                HSL_to_RGB(s_h, s_s, s_l, &r, &g, &b);
// Note: changed MapRGBA from using fmt to dst format because it is writing to
// dst. This is a meaningful distinction because even though the format enums
// are guaranteed to be the same, the palettes are not.
#if SDL_VERSION_ATLEAST(3, 0, 0)
                pixel = SDL_MapRGBA(dst_format, dst_palette, r, g, b, a);
#else
                pixel = SDL_MapRGBA(dst_format, r, g, b, a);
#endif
...
void
#if SDL_VERSION_ATLEAST(3, 0, 0)
laplacian(SDL_Surface *surf, const SDL_PixelFormatDetails *format,
          SDL_Surface *destsurf, const SDL_PixelFormatDetails *destformat)
#else
laplacian(SDL_Surface *surf, SDL_PixelFormat *format, SDL_Surface *destsurf,
          SDL_PixelFormat *destformat)
#endif
{
    int ii;
    ...
    */
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            for (ii = 0; ii < 9; ii++) {
#if SDL_VERSION_ATLEAST(3, 0, 0)
                SDL_GetRGBA(sample[ii], format, SDL_GetSurfacePalette(surf),
                            &c1r, &c1g, &c1b, &c1a);
#else
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
            SDL_GetRGBA(sample[4], format, SDL_GetSurfacePalette(surf), &c1r,
                        &c1g, &c1b, &c1a);
#else
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
            the_color =
                SDL_MapRGBA(format, SDL_GetSurfacePalette(surf), acolor[0],
                            acolor[1], acolor[2], acolor[3]);
#else
            the_color = SDL_MapRGBA(format, acolor[0], acolor[1], acolor[2],
                                    acolor[3]);
#endif
...
void
#if SDL_VERSION_ATLEAST(3, 0, 0)
average_color(SDL_Surface *surf, const SDL_PixelFormatDetails *format, int x,
              int y, int width, int height, Uint8 *r, Uint8 *g, Uint8 *b,
              Uint8 *a, SDL_bool consider_alpha)
#else
average_color(SDL_Surface *surf, SDL_PixelFormat *format, int x, int y,
...
void
#if SDL_VERSION_ATLEAST(3, 0, 0)
invert_non_simd(SDL_Surface *src, const SDL_PixelFormatDetails *src_format,
                SDL_Surface *newsurf,
                const SDL_PixelFormatDetails *newsurf_format)
#else
invert_non_simd(SDL_Surface *src, SDL_PixelFormat *src_format,
                SDL_Surface *newsurf, SDL_PixelFormat *newsurf_format)
#endif
{
    int x, y;
...
#if SDL_VERSION_ATLEAST(3, 0, 0)
            SDL_GetRGBA(pixel, src_format, src_palette, &r, &g, &b, &a);
            Uint32 new_pixel =
                SDL_MapRGBA(newsurf_format, newsurf_palette, ~r, ~g, ~b, a);
#else
            SDL_GetRGBA(pixel, src_format, &r, &g, &b, &a);
            Uint32 new_pixel = SDL_MapRGBA(newsurf_format, ~r, ~g, ~b, a);
#endif
            SURF_SET_AT(new_pixel, newsurf, x, y, (Uint8 *)newsurf->pixels,
                        newsurf_format, pix);
        }
